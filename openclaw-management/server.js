...
const UPGRADE_URL = process.env.UPGRADE_SERVICE_URL || "https://local.amyclaw.cn/upgrade";
const UPGRADE_PAGE_URL = process.env.UPGRADE_PAGE_URL || UPGRADE_URL;
const REMOTE_MANAGEMENT_URL = process.env.REMOTE_MANAGEMENT_URL || "https://local.amyclaw.cn";
const REMOTE_REGISTER_URL = process.env.REMOTE_REGISTER_URL || "https://remote.amyclaw.cn";
...
/** 校验磁盘上的 openclaw.json 是否已无敏感项（飞书帐号、provider apiKey、记忆 apiKey）。用于一键清除后验证落盘，未通过则触发补清。 */
...
const ROUTES = {
  ...
  "POST /api/clear-all-accounts": async (req, res) => {
    ...
    try {
      ...
      if (restarted) {
        send(res, 200, { ok: true, message: "已清除 Gateway Token、飞书帐号、大模型与记忆密钥及 .env 中对应敏感项；已重启 Gateway，主机进程内存中的旧配置已释放。" });
      } else {
        triggerGatewayReload().catch(() => {});
