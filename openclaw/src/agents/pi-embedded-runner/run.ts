  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      const applyApiKeyInfo = async (candidate?: string): Promise<void> => {
        ...
        // idatabase.ai expects Authorization: Bearer; pi-ai/google SDK uses x-goog-api-key by default.
        // Inject Bearer into model.headers so createClient() in pi-ai uses it.
        if (
          runtimeModel.api === "google-generative-ai" &&
          runtimeModel.baseUrl?.includes("idatabase.ai") &&
          apiKeyInfo.apiKey
        ) {
          const headers = {
            ...(typeof runtimeModel.headers === "object" && !Array.isArray(runtimeModel.headers)
              ? runtimeModel.headers
              : {}),
            Authorization: `Bearer ${apiKeyInfo.apiKey}`,
          };
          runtimeModel = { ...runtimeModel, headers };
          effectiveModel = { ...effectiveModel, headers };
        }
