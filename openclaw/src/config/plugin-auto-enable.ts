...
function isPluginExplicitlyDisabled(cfg: OpenClawConfig, pluginId: string): boolean {
  ...
  // plugins.entries.<id>.enabled=false alone is ambiguous: the UI may leave it off while
  // channels.<id>.enabled=true (extension still won't load). Treat that as misconfiguration
  // so applyPluginAutoEnable can align entries; only honor explicit opt-out when the
  // channel is not enabled.
  if (builtInChannelId) {
    const channelsRoot = cfg.channels as Record<string, unknown> | undefined;
    const ch = channelsRoot?.[builtInChannelId];
    const channelOn =
      ch &&
      typeof ch === "object" &&
      !Array.isArray(ch) &&
      (ch as { enabled?: unknown }).enabled === true;
    if (channelOn) {
      return false;
    }
  }
...
export function applyPluginAutoEnable(params: {
  ...
  for (const entry of configured) {
    ...
    // Channel plugins need BOTH channels.<id>.enabled and plugins.entries.<id>.enabled
    // to load. Previously we only checked the channel flag, so users could have
    // channels.feishu.enabled=true while plugins.entries.feishu.enabled=false and the
    // extension would never register.
    const alreadyEnabled =
      builtInChannelId != null
        ? (() => {
            ...
            const channelOn = (channelConfig as { enabled?: unknown }).enabled === true;
            const pluginOn = next.plugins?.entries?.[entry.pluginId]?.enabled === true;
            return channelOn && pluginOn;
          })()
