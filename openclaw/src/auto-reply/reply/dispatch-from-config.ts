...
export async function dispatchReplyFromConfig(params: {
  ...
  try {
    ...
    // When agent produced no final reply and no block content, send a fallback so the user sees a response (e.g. Feishu typing-then-nothing).
    if (replies.length === 0 && blockCount === 0) {
      const fallbackPayload: ReplyPayload = {
        text: "暂时无法生成回复，请检查模型/API 配置或稍后再试。",
      };
      getLogger().info(
        { sessionKey: ctx.SessionKey, channel: ctx.OriginatingChannel ?? ctx.Surface ?? ctx.Provider },
        "dispatch-from-config: sending fallback reply (agent produced no payload)",
      );
      if (shouldRouteToOriginating && originatingChannel && originatingTo) {
        const result = await routeReply({
        ...
        dispatcher.sendFinalReply(fallbackPayload);
      }
    }

    const counts = dispatcher.getQueuedCounts();
    counts.final += routedFinalCount;
    if (counts.final === 0 && replies.length === 0) {
      getLogger().warn(
        {
          sessionKey: ctx.SessionKey,
        ...
        "dispatch-from-config: agent completed with no final reply (replies=0); check model/API config and gateway logs for LLM errors",
      );
    }
    recordProcessed(
      "completed",
