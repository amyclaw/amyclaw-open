...
export async function runReplyAgent(params: {
  ...
  try {
    ...
    if (payloadArray.length === 0) {
      logVerbose(
        `agent-runner: model run produced no payloads (sessionKey=${sessionKey ?? "unknown"}); possible causes: empty model response, API error, or no LLM config`,
      );
    ...
    if (replyPayloads.length === 0) {
      logVerbose(
        `agent-runner: buildReplyPayloads produced no deliverable replies (sessionKey=${sessionKey ?? "unknown"})`,
      );
      return finalizeWithFollowup(undefined, queueKey, runFollowupTurn);
    }
