...
import { Mi as DISCORD_DEFAULT_INBOUND_WORKER_TIMEOUT_MS, Ni as DISCORD_DEFAULT_LISTENER_TIMEOUT_MS, iw as stripUrlUserInfo } from "./auth-profiles-Bc6TPi0n.js";
import { I as sensitive } from "./zod-schema.core-CWxzqcUs.js";
...
const FIELD_HELP = {
	...
	"gateway.channelStaleEventThresholdMinutes": "How many minutes a connected channel can go without receiving any event before the health monitor treats it as a stale socket and triggers a restart. Default: 30.",
	...
	"agents.list[].runtime.acp.agent": "Optional ACP harness agent id to use for this OpenClaw agent (for example codex, claude).",
	...
	"browser.profiles.*.userDataDir": "Per-profile Chromium user data directory for existing-session attachment through Chrome DevTools MCP. Use this for host-local Brave, Edge, Chromium, or non-default Chrome profiles when the built-in auto-connect path would pick the wrong browser data directory.",
	"browser.profiles.*.driver": "Per-profile browser driver mode. Use \"openclaw\" (or legacy \"clawd\") for CDP-based profiles, or use \"existing-session\" for host-local Chrome DevTools MCP attachment.",
	...
	"gateway.push.apns.relay": "External relay settings for relay-backed APNs sends. The gateway uses this relay for push.test, wake nudges, and reconnect wakes after a paired official iOS build publishes a relay-backed registration.",
	"gateway.push.apns.relay.baseUrl": "Base HTTPS URL for the external APNs relay service used by official/TestFlight iOS builds. Keep this aligned with the relay URL baked into the iOS build so registration and send traffic hit the same deployment.",
	...
	"gateway.reload.deferralTimeoutMs": "Maximum time (ms) to wait for in-flight operations to complete before forcing a SIGUSR1 restart. Default: 300000 (5 minutes). Lower values risk aborting active subagent LLM calls.",
	...
	"tools.web.search.provider": "Search provider (\"brave\", \"firecrawl\", \"gemini\", \"grok\", \"kimi\", or \"perplexity\"). Auto-detected from available API keys if omitted.",
	...
	"tools.web.search.firecrawl.apiKey": "Firecrawl API key for web search (fallback: FIRECRAWL_API_KEY env var).",
	"tools.web.search.firecrawl.baseUrl": "Firecrawl Search base URL override (default: \"https://api.firecrawl.dev\").",
	...
	"tools.web.search.perplexity.apiKey": "Perplexity or OpenRouter API key (fallback: PERPLEXITY_API_KEY or OPENROUTER_API_KEY env var). Direct Perplexity keys default to the Search API; OpenRouter keys use Sonar chat completions.",
	"tools.web.search.perplexity.baseUrl": "Optional Perplexity/OpenRouter chat-completions base URL override. Setting this opts Perplexity into the legacy Sonar/OpenRouter compatibility path.",
	"tools.web.search.perplexity.model": "Optional Sonar/OpenRouter model override (default: \"perplexity/sonar-pro\"). Setting this opts Perplexity into the legacy chat-completions compatibility path.",
	...
	"agents.defaults.bootstrapPromptTruncationWarning": "Inject agent-visible warning text when bootstrap files are truncated: \"off\", \"once\" (default), or \"always\".",
	...
	"agents.defaults.memorySearch.outputDimensionality": "Gemini embedding-2 only: chooses the output vector size for memory embeddings. Use 768, 1536, or 3072 (default), and expect a full reindex when you change it because stored vector dimensions must stay consistent.",
	...
	"plugins.installs.*.marketplaceName": "Marketplace display name recorded for marketplace-backed plugin installs (if available).",
	"plugins.installs.*.marketplaceSource": "Original marketplace source used to resolve the install (for example a repo path or Git URL).",
	"plugins.installs.*.marketplacePlugin": "Plugin entry name inside the source marketplace, used for later updates.",
	...
	"agents.defaults.compaction.qualityGuard": "Optional quality-audit retry settings for safeguard compaction summaries. Leave this disabled unless you explicitly want summary audits and one-shot regeneration on failed checks.",
	"agents.defaults.compaction.qualityGuard.enabled": "Enables summary quality audits and regeneration retries for safeguard compaction. Default: false, so safeguard mode alone does not turn on retry behavior.",
	...
	"agents.defaults.compaction.postCompactionSections": "AGENTS.md H2/H3 section names re-injected after compaction so the agent reruns critical startup guidance. Leave unset to use \"Session Startup\"/\"Red Lines\" with legacy fallback to \"Every Session\"/\"Safety\"; set to [] to disable reinjection entirely.",
	...
	"channels.telegram.execApprovals": "Telegram-native exec approval routing and approver authorization. Enable this only when Telegram should act as an explicit exec-approval client for the selected bot account.",
	"channels.telegram.execApprovals.enabled": "Enable Telegram exec approvals for this account. When false or unset, Telegram messages/buttons cannot approve exec requests.",
	"channels.telegram.execApprovals.approvers": "Telegram user IDs allowed to approve exec requests for this bot account. Use numeric Telegram user IDs; prompts are only delivered to these approvers when target includes dm.",
	"channels.telegram.execApprovals.agentFilter": "Optional allowlist of agent IDs eligible for Telegram exec approvals, for example `[\"main\", \"ops-agent\"]`. Use this to keep approval prompts scoped to the agents you actually operate from Telegram.",
	"channels.telegram.execApprovals.sessionFilter": "Optional session-key filters matched as substring or regex-style patterns before Telegram approval routing is used. Use narrow patterns so Telegram approvals only appear for intended sessions.",
	"channels.telegram.execApprovals.target": "Controls where Telegram approval prompts are sent: \"dm\" sends to approver DMs (default), \"channel\" sends to the originating Telegram chat/topic, and \"both\" sends to both. Channel delivery exposes the command text to the chat, so only use it in trusted groups/topics.",
	...
	"channels.slack.capabilities.interactiveReplies": "Enable agent-authored Slack interactive reply directives (`[[slack_buttons: ...]]`, `[[slack_select: ...]]`). Default: false.",
	...
	"channels.discord.allowBots": "Allow bot-authored messages to trigger Discord replies (default: false). Set \"mentions\" to only accept bot messages that mention the bot.",
	...
	"messages.statusReactions.emojis": "Override default status reaction emojis. Keys: thinking, compacting, tool, coding, web, done, error, stallSoft, stallHard. Must be valid Telegram reaction emojis.",
	...
	"channels.telegram.silentErrorReplies": "When true, Telegram bot replies marked as errors are sent silently (no notification sound). Default: false.",
	"channels.telegram.threadBindings.enabled": "Enable Telegram conversation binding features (/focus, /unfocus, /agents, and /session idle|max-age). Overrides session.threadBindings.enabled when set.",
	"channels.telegram.threadBindings.idleHours": "Inactivity window in hours for Telegram bound sessions. Set 0 to disable idle auto-unfocus (default: 24). Overrides session.threadBindings.idleHours when set.",
	"channels.telegram.threadBindings.maxAgeHours": "Optional hard max age in hours for Telegram bound sessions. Set 0 to disable hard cap (default: 0). Overrides session.threadBindings.maxAgeHours when set.",
	"channels.telegram.threadBindings.spawnSubagentSessions": "Allow subagent spawns with thread=true to auto-bind Telegram current conversations when supported.",
	"channels.telegram.threadBindings.spawnAcpSessions": "Allow ACP spawns with thread=true to auto-bind Telegram current conversations when supported.",
	...
	"channels.discord.autoPresence.enabled": "Enable automatic Discord bot presence updates based on runtime/model availability signals. When enabled: healthy=>online, degraded/unknown=>idle, exhausted/unavailable=>dnd.",
	"channels.discord.autoPresence.intervalMs": "How often to evaluate Discord auto-presence state in milliseconds (default: 30000).",
	"channels.discord.autoPresence.minUpdateIntervalMs": "Minimum time between actual Discord presence update calls in milliseconds (default: 15000). Prevents status spam on noisy state changes.",
	"channels.discord.autoPresence.healthyText": "Optional custom status text while runtime is healthy (online). If omitted, falls back to static channels.discord.activity when set.",
	"channels.discord.autoPresence.degradedText": "Optional custom status text while runtime/model availability is degraded or unknown (idle).",
	"channels.discord.autoPresence.exhaustedText": "Optional custom status text while runtime detects exhausted/unavailable model quota (dnd). Supports {reason} template placeholder.",
...
const FIELD_LABELS = {
	...
	"tools.web.search.firecrawl.apiKey": "Firecrawl Search API Key",
