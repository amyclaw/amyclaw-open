import { JC as buildRuntimeAccountStatusSnapshot, LT as collectAllowlistProviderRestrictSendersWarnings, ZC as createDefaultChannelRuntimeState, qC as buildProbeChannelStatusSummary, ww as PAIRING_APPROVED_MESSAGE } from "./auth-profiles-Bc6TPi0n.js";
...
import { a as parseMSTeamsConversationId, c as resolveMSTeamsUserAllowlist, i as normalizeMSTeamsUserInput, m as resolveMSTeamsCredentials, n as msteamsSetupAdapter, o as parseMSTeamsTeamChannelInput, r as normalizeMSTeamsMessagingTarget, s as resolveMSTeamsChannelAllowlist, t as msteamsSetupWizard } from "./msteams-CrzqFQ29.js";
import { r as resolveMSTeamsGroupToolPolicy } from "./policy-6YbLbz3e.js";
...
const loadMSTeamsChannelRuntime = createLazyRuntimeSurface(() => import("./channel.runtime-Bbsr5pH9.js"), ({ msTeamsChannelRuntime }) => msTeamsChannelRuntime);
const msteamsPlugin = {
	id: "msteams",
	meta: {
		...meta,
	...
	actions: {
		listActions: ({ cfg }) => {
			if (!(cfg.channels?.msteams?.enabled !== false && Boolean(resolveMSTeamsCredentials(cfg.channels?.msteams)))) return [];
			return ["poll"];
		},
		getCapabilities: ({ cfg }) => {
			return cfg.channels?.msteams?.enabled !== false && Boolean(resolveMSTeamsCredentials(cfg.channels?.msteams)) ? ["cards"] : [];
		},
	...
	gateway: { startAccount: async (ctx) => {
		const { monitorMSTeamsProvider } = await import("./src-Cw3qYz02.js");
