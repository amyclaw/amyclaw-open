import { Bh as listFeishuAccountIds, Eh as parseFeishuConversationId, Hh as resolveFeishuAccount, Ih as createFeishuClient, JC as buildRuntimeAccountStatusSnapshot, LT as collectAllowlistProviderRestrictSendersWarnings, Mh as feishuSetupWizard, Nh as feishuSetupAdapter, Uh as resolveFeishuCredentials, Vh as resolveDefaultFeishuAccountId, ZC as createDefaultChannelRuntimeState, gT as mapAllowFromEntries, ig as buildSecretInputSchema, qC as buildProbeChannelStatusSummary, ww as PAIRING_APPROVED_MESSAGE, zh as listEnabledFeishuAccounts } from "./auth-profiles-Bc6TPi0n.js";
...
import { k as createActionGate } from "./provider-web-search-DqPBRERs.js";
...
import { i as normalizeFeishuTarget, r as looksLikeFeishuId, t as getFeishuRuntime } from "./runtime-BBTvy_Yh.js";
import { n as listFeishuDirectoryPeers, t as listFeishuDirectoryGroups } from "./directory.static-BlX--T0S.js";
import { i as resolveFeishuGroupToolPolicy } from "./policy-BrYGTmER.js";
...
const loadFeishuChannelRuntime = createLazyRuntimeSurface(() => import("./channel.runtime-CRFN2sqH.js"), ({ feishuChannelRuntime }) => feishuChannelRuntime);
...
const feishuPlugin = {
	...
	actions: {
		...
		getCapabilities: ({ cfg }) => {
			return cfg.channels?.feishu?.enabled !== false && Boolean(resolveFeishuCredentials(cfg.channels?.feishu)) ? ["cards"] : [];
		},
	...
	acpBindings: {
		normalizeConfiguredBindingTarget: ({ conversationId }) => normalizeFeishuAcpConversationId(conversationId),
		matchConfiguredBinding: ({ bindingConversationId, conversationId, parentConversationId }) => matchFeishuAcpConversation({
	...
	directory: {
		...
		listPeersLive: async ({ cfg, query, limit, accountId }) => (await loadFeishuChannelRuntime()).listFeishuDirectoryPeersLive({
			cfg,
			query: query ?? void 0,
			limit: limit ?? void 0,
			accountId: accountId ?? void 0
		}),
		listGroupsLive: async ({ cfg, query, limit, accountId }) => (await loadFeishuChannelRuntime()).listFeishuDirectoryGroupsLive({
			cfg,
			query: query ?? void 0,
			limit: limit ?? void 0,
			accountId: accountId ?? void 0
	...
	outbound: {
		...
		sendMedia: async (params) => (await loadFeishuChannelRuntime()).feishuOutbound.sendMedia(params)
	},
	status: {
	...
	gateway: { startAccount: async (ctx) => {
		const { monitorFeishuProvider } = await import("./monitor-DKhE9_-r.js");
		const account = resolveFeishuAccount({
			cfg: ctx.cfg,
