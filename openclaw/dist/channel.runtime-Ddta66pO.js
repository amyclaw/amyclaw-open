...
import { Cg as resolveSetupAccountId, XS as resolveDefaultDiscordAccountId, ZS as resolveDiscordAccount, vg as promptLegacyChannelAllowFrom } from "./auth-profiles-Bc6TPi0n.js";
...
import { i as parseDiscordAllowFromId, t as createDiscordSetupWizardBase } from "./setup-core-DwZ_G2We.js";
...
async function resolveDiscordGroupAllowlist(params) {
	const token = resolveDiscordAccount({
		cfg: params.cfg,
		accountId: params.accountId
	}).token || (typeof params.credentialValues.token === "string" ? params.credentialValues.token : "");
	if (!token || params.entries.length === 0) return params.entries.map((input) => ({
		input,
		resolved: false
	}));
...
const discordSetupWizard = { ...createDiscordSetupWizardBase({
	...
	resolveAllowFromEntries: async ({ cfg, accountId, credentialValues, entries }) => await resolveDiscordAllowFromEntries({
		token: resolveDiscordAccount({
			cfg,
			accountId
		}).token || (typeof credentialValues.token === "string" ? credentialValues.token : ""),
		entries
	}),
