...
import { i as formatUncaughtError } from "./errors-DOJWZqNo.js";
import { t as isMainModule } from "./is-main-DRn0Pf7d.js";
import { C as getCommandPositionalsWithRootOptions, D as getVerboseFlag, E as getPrimaryCommand, F as isValueToken, O as hasFlag, S as getCommandPathWithRootOptions, T as getPositiveIntFlagValue, j as isRootHelpInvocation, k as hasHelpOrVersion, w as getFlagValue } from "./logger-ByBU4z1U.js";
...
import { a as enableConsoleCapture, m as defaultRuntime } from "./subsystem-MGyxt_Bl.js";
import "./ansi-BPhP6LBZ.js";
import "./boolean-D8Ha5nYV.js";
import { r as normalizeEnv, t as isTruthyEnvValue } from "./env-DlREndPb.js";
import { n as VERSION } from "./version-BMIQmWNJ.js";
import "./logging-BdFqMomc.js";
import { t as assertSupportedRuntime } from "./runtime-guard-CmEI5MNY.js";
import { t as ensureOpenClawCliOnPath } from "./path-env-B-B47KLG.js";
import { t as emitCliBanner } from "./banner-8QYyJMHk.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-D2fZQGbr.js");
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		if (timeoutMs === null) return false;
		if (json) {
			const { statusJsonCommand } = await import("./status-json-8bEPksQb.js");
		...
		const { statusCommand } = await import("./status-CS3FMJgl.js");
		await statusCommand({
			json,
			deep,
			all,
...
const routeGatewayStatus = {
	...
	run: async (argv) => {
		...
		const { runDaemonStatus } = await import("./status-BI1lpIXB.js");
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-b0c-wRH5.js");
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-B24JGAXi.js");
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./memory-cli-CurMJ8db.js");
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-C0x0d6fT.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-C0x0d6fT.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-DjdJ35fi.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-DjdJ35fi.js");
...
async function prepareRoutedCommand(params) {
	...
	const { ensureConfigReady } = await import("./config-guard-D748XcgI.js");
	...
	if (typeof params.loadPlugins === "function" ? params.loadPlugins(params.argv) : params.loadPlugins) {
		const { ensurePluginRegistryLoaded } = await import("./plugin-registry-CBK7Y86U.js");
...
async function closeCliMemoryManagers() {
	try {
		const { closeAllMemorySearchManagers } = await import("./search-manager-DdJ-1N3h.js");
...
async function runCli(argv = process$1.argv) {
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputRootHelp } = await import("./root-help-CoHg-3qq.js");
			outputRootHelp();
			return;
		}
		if (await tryRouteCli(normalizedArgv)) return;
		enableConsoleCapture();
		const { buildProgram } = await import("./program-Bwzc3gTt.js");
		const program = buildProgram();
		const { installUnhandledRejectionHandler } = await import("./unhandled-rejections-a5XFSANd.js");
		...
		if (primary) {
			const { getProgramContext } = await import("./program-context-C6wNe2H4.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-DlVDaxit.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-CyVzUgfT.js");
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { loadValidatedConfigForPluginRegistration } = await import("./register.subclis-CyVzUgfT.js");
