...
import { M as isRootVersionInvocation, j as isRootHelpInvocation, k as hasHelpOrVersion } from "./logger-ByBU4z1U.js";
...
if (!isMainModule({
	...
	const { installGaxiosFetchCompat } = await import("./gaxios-fetch-compat-Cgtx8Se2.js");
	...
	function tryHandleRootVersionFastPath(argv) {
		...
		Promise.all([import("./version-oR9g_sID.js"), import("./git-commit-CNiECVcl.js")]).then(([{ VERSION }, { resolveCommitHash }]) => {
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	import("./root-help-CoHg-3qq.js").then(({ outputRootHelp }) => {
		outputRootHelp();
	}).catch(handleError);
	return true;
}
function runMainOrRootHelp(argv) {
	...
	import("./run-main-Dlf0idRH.js").then(({ runCli }) => runCli(argv)).catch((error) => {
