import { n as DEFAULT_GATEWAY_PORT, u as resolveGatewayPort } from "./paths-1qR_mW4i.js";
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-Lms65jAX.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-kA4h3Owq.js");
		const { logConfigUpdated } = await import("./logging-UfkPcntI.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-NZJ4xSw3.js");
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-BB5Gkflr.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-Cq0sSPfw.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-akHe0DTX.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-C9YMhyj_.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-CRFn5UTb.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-DrNGlw07.js");
		const { setupChannels } = await import("./onboard-channels-CsE98nTY.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-BDzLV_vq.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-BatyLpBe.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-CAL6ETgl.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-gCJ8u5oz.js");
