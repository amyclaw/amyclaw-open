...
import { Ag as setTopLevelChannelDmPolicyWithAllowFrom, Mg as splitSetupEntries, gg as parseSetupEntriesWithParser, kg as setTopLevelChannelAllowFrom, ug as mergeAllowFromEntries } from "./auth-profiles-Bc6TPi0n.js";
import { n as normalizeAccountId$1, r as normalizeOptionalAccountId, t as DEFAULT_ACCOUNT_ID } from "./account-id-O4Og6DrK.js";
import { a as DmPolicySchema, m as MarkdownConfigSchema } from "./zod-schema.core-CWxzqcUs.js";
import { r as buildChannelConfigSchema, t as AllowFromListSchema } from "./config-schema-GQ6uWjXe.js";
...
function patchNostrConfig(params) {
	const nextNostr = { ...params.cfg.channels?.nostr ?? {} };
	for (const field of params.clearFields ?? []) delete nextNostr[field];
	return {
		...params.cfg,
		channels: {
			...params.cfg.channels,
...
function setNostrDmPolicy(cfg, dmPolicy) {
...
function setNostrAllowFrom(cfg, allowFrom) {
...
const nostrSetupAdapter = {
	...
	applyAccountName: ({ cfg, name }) => patchNostrConfig({
		cfg,
...
const nostrSetupWizard = {
	...
	envShortcut: {
		...
		apply: async ({ cfg }) => patchNostrConfig({
			cfg,
			enabled: true,
	...
	credentials: [{
		...
		applyUseEnv: async ({ cfg }) => patchNostrConfig({
			cfg,
			enabled: true,
			clearFields: ["privateKey"],
			patch: {}
		}),
		applySet: async ({ cfg, resolvedValue }) => patchNostrConfig({
			cfg,
			enabled: true,
	...
	disable: (cfg) => patchNostrConfig({
		cfg,
		patch: { enabled: false }
	})
};
//#endregion
export { resolveNostrAccount as a, getNostrRuntime as c, NostrConfigSchema as d, NostrProfileSchema as f, resolveDefaultNostrAccountId as i, setNostrRuntime as l, verifyEvent as m, nostrSetupWizard as n, normalizePubkey as o, SimplePool as p, listNostrAccountIds as r, startNostrBus as s, nostrSetupAdapter as t, contentToProfile as u };
