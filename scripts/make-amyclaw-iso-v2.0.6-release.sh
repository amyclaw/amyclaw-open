#!/usr/bin/env bash
# 母机打 AmyClaw v2.0.6-release ISO：写入版本号并调用 eggs-produce-and-backup.sh
# 打 ISO 前仍须完成：mother-image-prepare、deploy-to-opt、apply-netplan（见 docs/eggs-量产克隆与使用说明.md）
#
# 用法：sudo bash /mnt/disk/amyclaw/scripts/make-amyclaw-iso-v2.0.6-release.sh
set -euo pipefail
[[ $(id -u) -eq 0 ]] || { echo "请使用 sudo 运行"; exit 1; }

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
VER="${AMYCLAW_IMAGE_VERSION:-v2.0.6-release}"
