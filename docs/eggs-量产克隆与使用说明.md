...
**当前镜像版本：v2.0.7-release**（ISO 文件名形如 `amyclaw_v2.0.7-release_amd64_<日期时间>.iso`，由 `AMYCLAW_IMAGE_VERSION` 控制。）
...
| 项目 | 说明 |
|------|------|
| **本次生成的 ISO** | eggs 默认名 **`amyclaw_amd64_<日期>_<时间>.iso`**；脚本会额外复制为 **`amyclaw_v2.0.7-release_amd64_<日期>_<时间>.iso`**（版本号由环境变量 `AMYCLAW_IMAGE_VERSION` 控制，默认 **v2.0.7-release**） |
| **备份位置** | `/mnt/backup/`（体积随系统变化，约数 GB） |
| **镜像版本标记** | 母机 **`/etc/amyclaw-release`**（`AMYCLAW_IMAGE_VERSION=v2.0.7-release`），打入 ISO 后克隆机可读 |
...
# 4) 写入镜像版本号（可选，默认 v2.0.7-release）
echo "AMYCLAW_IMAGE_VERSION=v2.0.7-release" | sudo tee /etc/amyclaw-release
echo "BUILD_DATE=$(date -Iseconds)" | sudo tee -a /etc/amyclaw-release

# 5) 生成 ISO 并复制到 /mnt/backup（带版本文件名）
sudo AMYCLAW_IMAGE_VERSION=v2.0.7-release /mnt/disk/amyclaw/scripts/eggs-produce-and-backup.sh

# 或一键：写入 /etc/amyclaw-release 为 v2.0.7-release 并打 ISO（内部仍调用 eggs-produce-and-backup.sh）
# sudo bash /mnt/disk/amyclaw/scripts/make-amyclaw-iso-v2.0.7-release.sh
```

- 会生成新 ISO（eggs 默认名 + **`amyclaw_v2.0.7-release_amd64_日期时间.iso`**），并复制到 `/mnt/backup`。
...
1. **获取 ISO**  
   从母机或备份处拷贝 **`amyclaw_v2.0.7-release_amd64_*.iso`**（或 eggs 默认生成的 `amyclaw_amd64_*.iso`）。
...
- **需要更新镜像**  
  在母机按 **「二、母机再次打包」** 全流程执行（至少 **`deploy-to-opt.sh`** + 写入 **`/etc/amyclaw-release`** + **`eggs-produce-and-backup.sh`**；默认 **`AMYCLAW_IMAGE_VERSION=v2.0.7-release`**）。勿只跑 `eggs produce` 而跳过 deploy，否则 `/opt/amyclaw` 可能不是最新。用生成于 **`/mnt/backup/`** 的新 ISO 重新刻录 U 盘即可。
...
- **ISO 备份目录**：`/mnt/backup/`（如 eggs 默认名 `amyclaw_amd64_*.iso`，及带版本副本 **`amyclaw_v2.0.7-release_amd64_*.iso`**）
...
**姊妹篇（与 v2.0.7-release 配套；不替代本文）**

- **`AmyClaw-量产与运维-v2.0.7-release.md`**：量产/运维一页总览（**`AmyClaw-量产与运维-v2.0-release.md` 已迁移至此**）。  
- **`AmyClaw-用户安装后使用说明-v2.0.7-release.md`**：仅面向安装后的用户（**`AmyClaw-用户安装后使用说明-v2.0-release.md` 为跳转**）。  
