...
| **版本号** | 环境变量 **`AMYCLAW_IMAGE_VERSION`**（当前量产默认 **`v2.0.3`**），写入 **`/etc/amyclaw-release`** 后打入镜像。 |
...
4. 写入 **`/etc/amyclaw-release`**（可选，建议 **`AMYCLAW_IMAGE_VERSION=v2.0.3`**）。  
...
（用户侧简要说明见 **`AmyClaw-用户安装后使用说明-v2.0.3.md`**；飞书端注册与权限见 **`飞书-开放平台注册应用与机器人指南.md`**。）
...
| `AmyClaw-用户安装后使用说明-v2.0.3.md` | **终端用户**：仅安装后设置与使用。 |
| `飞书-开放平台注册应用与机器人指南.md` | **飞书端**：注册应用与机器人。 |

---

*文档版本：v2.0.3 · 与镜像 `AMYCLAW_IMAGE_VERSION` 及仓库内其它文档并列维护*
