# AmyClaw 量产与运维说明 v2.0.6-release
...
| **版本号** | 环境变量 **`AMYCLAW_IMAGE_VERSION`**（当前量产默认 **`v2.0.6-release`**），写入 **`/etc/amyclaw-release`** 后打入镜像。 |
...
4. 写入 **`/etc/amyclaw-release`**（可选，建议 **`AMYCLAW_IMAGE_VERSION=v2.0.6-release`**）。  
...
**一键打 ISO（推荐）**：`sudo bash /mnt/disk/amyclaw/scripts/make-amyclaw-iso-v2.0.6-release.sh`（内部写入 `/etc/amyclaw-release` 并调用 `eggs-produce-and-backup.sh`）。
...
（用户侧简要说明见 **`AmyClaw-用户安装后使用说明-v2.0.6-release.md`**；飞书端注册与权限见 **`飞书-开放平台注册应用与机器人指南.md`**。）
...
## 6. v2.0.6-release 相对 v2.0.3 的变更摘要（运维与管理页相关）
...
| `AmyClaw-用户安装后使用说明-v2.0.6-release.md` | **终端用户**：仅安装后设置与使用。 |
| `飞书-开放平台注册应用与机器人指南.md` | **飞书端**：注册应用与机器人。 |

---

*文档版本：v2.0.6-release · 与镜像 `AMYCLAW_IMAGE_VERSION` 及仓库内其它文档并列维护*
